...
namespace GodLightning
{
    public class Plugin : BasePlugin, IPluginConfig<Configs>
    {
        public override string ModuleName => "The God of Lightning Acheivement";
        ...
        public Dictionary<CCSPlayerController, PlayerZeusKillCount> _playerKillCount { get; set; } = new();
        ...
        public void CountKill(CCSPlayerController client)
        {
            ...
            if (_playerKillCount[client!].KillCount >= Config.MaxZeusKillCount)
            {
                var steamid = client.AuthorizedSteamID?.SteamId64;
        ...
        public async Task TaskComplete(CCSPlayerController client, ulong steamid)
        {
            ...
            if (response != null)
            {
                ...
                Server.NextFrame(() =>
                {
                    client.PrintToChat($" {ChatColors.Green}[Acheivement]{ChatColors.Default} You acheive 'The God of Lightning ' (Kill enemies with Zeus for {Config.MaxZeusKillCount} times)");
